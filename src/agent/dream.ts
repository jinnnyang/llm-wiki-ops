...
No \`compression\` field on a dream page. The ladder (active → condensed → skeleton) models knowledge slowly decaying as nobody reads it; a dream is not knowledge and does not decay that way. It has two ends: a later verification settles it and its content graduates into real nodes, or it is judged worthless and deleted outright. Writing \`compression: active\` on it implies a four-step decay it will never walk.
...
- Dream pages do not use the ladder at all: they carry no \`compression\` field. A dream is a question, so it is either settled (its content graduates into knowledge nodes) or worthless (delete_node outright). Never assign one a compression stage.
